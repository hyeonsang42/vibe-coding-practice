              ...
              <div className="flex items-start gap-3.5">
                <span
                  className={`tnum mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[0.6875rem] font-medium ${
                    checked ? "bg-accent text-ink" : "bg-line text-muted"
                  }`}
                >
                  {index + 1}
                </span>
                ...
                <div className="flex-1">
                  ...
                  {checked ? (
                    ...
                    <button
                      ...
                      className="mt-3 inline-flex h-10 items-center justify-center rounded-sm bg-accent px-4 text-[0.8125rem] font-medium text-ink transition hover:opacity-90"
                    >
                      여기서 QR 찍기
...
function Badge() {
  return (
    <section className="rise-in mt-8 rounded-sm border border-accent/40 bg-accent-soft px-6 py-7 text-center">
      <p className="eyebrow text-accent">코스 완주</p>

      <div className="mx-auto mt-4 flex h-20 w-20 items-center justify-center rounded-full border border-accent/50">
...
function ScanOverlay({ place }: { place: Place }) {
  return (
    <div
      role="status"
      aria-live="assertive"
      className="fixed inset-0 z-10 flex items-center justify-center bg-background/92 px-8"
    >
